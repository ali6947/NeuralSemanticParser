...
def main():
    ...
    all_sims=torch.nn.functional.cosine_similarity(cls_embs.unsqueeze(1),cls_embs.unsqueeze(0),dim=-1)
    all_sims[torch.arange(num_eg),torch.arange(num_eg)]=-100
    print(all_sims.shape)
    # with open('pickle_objs/bigbird_base/spider_train_train_cossim_no_leafs_double_check.pkl','wb') as fcos:
    #     pickle.dump(all_sims,fcos)

    for eg_idx in tqdm(range(num_eg)):
        accvec=all_sims[eg_idx]  
        sidx=np.argsort(accvec)[-11:]
        assert eg_idx not in sidx
    ...
    with open('pickle_objs/bigbird_base/spider_val_bigbird_ret_soft_cases_no_leafs_val_double_check.pkl','wb') as fw:
