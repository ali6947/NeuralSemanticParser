...
class SmbopParser(Model):
    def __init__(
        ...
        )._allennlp_tokenizer.tokenizer # NOTE: changed to bigbird, was originally grappa
        ...
        self._emb_to_action_dim_cases = torch.nn.Linear(
            in_features=self.q_emb_dim,
            out_features=self._action_dim,
        )
    ...
    def forward(
        ...
        is_gold_span=None,
        inst_id=None,
        cases=None #NOTE: cases added new, cases is like 'enc' but having the data items added one after the other
    ):
        ...
        beam_list = []
        item_list = []
        self.type_of_tr=np.random.uniform()
        ...
        (
            ...
            case_info,
            case_info_mask
        ) = self._encode_utt_schema(enc, offsets, relation, lengths,cases) #NOTE: cases added new
    ...
    def _augment_with_utterance(
        ...
        if self.type_of_tr<0.7:

            if self.disentangle_cntx:
                utt_w_cases=torch.cat([embedded_utterance,case_info],dim=1)
                utt_w_cases_mask=torch.cat([utterance_mask,case_info_mask],dim=1)

                enriched_beam_rep = self._utterance_augmenter(
                    beam_rep, utt_w_cases, ctx_att_mask=utt_w_cases_mask
                ...
                utt_w_cases=torch.stack([embedded_utterance,case_info],dim=1)
                utt_w_cases_mask=torch.stack([utterance_mask,case_info_mask],dim=1)
                encoder_input = torch.cat([utt_w_cases, beam_rep], dim=1)
                input_mask = torch.cat([utt_w_cases_mask.bool(), beam_mask.bool()], dim=-1)
                encoder_output = ctx(inputs=encoder_input, mask=input_mask)
                _, enriched_beam_rep = torch.split(
                    encoder_output, [utt_w_cases_mask.size(-1), beam_mask.size(-1)], dim=1
                )
    ...
    def emb_q_wcases(self,mask_dim1,cases,len_main_enc): #NOTE: made for big bird large, cases already has enc and the neighbours
        pad_dim = cases["tokens"]["mask"].size(-1)
        MAX_SEQLEN=1024
        if pad_dim > MAX_SEQLEN: #cases so long, no space for cases
            for key in cases["tokens"].keys():
                cases["tokens"][key] = cases["tokens"][key][:, :MAX_SEQLEN]

            embedded_utterance_schema = self._question_embedder(cases)
        else: 
            embedded_utterance_schema = self._question_embedder(cases)

        end_idx=embedded_utterance_schema.shape[-2]-1
        bs=embedded_utterance_schema.shape[0]
        end_idx_mat=(torch.ones(bs)*end_idx).to(self._device)
        case_spans=torch.stack([len_main_enc,end_idx_mat],dim=-1) # B x 2
        overflow_idx=torch.where(len_main_enc>end_idx)[0]
        case_spans=torch.clamp(case_spans,max=end_idx)
        case_spans=case_spans.unsqueeze(1).long() # B x 1 x 2
        try:
            case_info,case_info_mask=vec_utils.batched_span_select(embedded_utterance_schema,case_spans) # B x max span size x emb_dim , B x max span size
            if overflow_idx.shape[0]!=0:
                case_info_mask[overflow_idx]=False
        except:
            import pdb
            pdb.set_trace()

        return embedded_utterance_schema[:,:mask_dim1,:],case_info.squeeze(1),case_info_mask.squeeze(1)

    def _encode_utt_schema(self, enc, offsets, relation, lengths,cases): #NOTE: cases added new
        len_main_enc=offsets[torch.arange(lengths.shape[0]),lengths[:,1,1].long(),-1]+4 # B;  checked experimentally for bigbird

        if self.type_of_tr<0.7:
            embedded_utterance_schema,case_info,case_info_mask = self.emb_q_wcases(enc["tokens"]["mask"].size(-1),cases,len_main_enc) #NOTE: original was embq, without contiguous
        else:
            embedded_utterance_schema=self.emb_q(enc)
            case_info,case_info_mask=None,None

        embedded_utterance_schema=embedded_utterance_schema.contiguous()
        case_info=case_info.contiguous()
        case_info_mask=case_info_mask.contiguous()

        case_info=self._emb_to_action_dim_cases(case_info)
        ...
        return schema, schema_mask, embedded_utterance, utterance_mask,case_info,case_info_mask
