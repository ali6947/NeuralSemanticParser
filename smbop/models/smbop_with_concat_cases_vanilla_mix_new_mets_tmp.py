...
class SmbopParser(Model):
    ...
    def emb_q_wcases(self,mask_dim1,cases): #NOTE: made for big bird large, cases already has enc and the neighbours
        ...
        return embedded_utterance_schema[:,:mask_dim1,:]

    def _encode_utt_schema(self, enc, offsets, relation, lengths,cases): #NOTE: cases added new


        if np.random.uniform()<0.7 or not self.training :
            embedded_utterance_schema = (self.emb_q_wcases(enc["tokens"]["mask"].size(-1),cases)).contiguous() #NOTE: original was embq, without contiguous
        else:
            assert False
