...
def print_scores(scores, etype,fout):
...
def evaluate(gold, predict, db_dir, etype, kmaps,fout):
    # sys.stdout=open(fout.split('.')[0]+'_sqls.txt', "w")
    with open(gold) as f:
    ...
    # sys.stdout.close()
    print_scores(scores, etype,fout)
...
def eval_exec_match(db, p_str, g_str, pred, gold):
    ...
    """
    conn = sqlite3.connect(db)
    conn.text_factory = str ## NOTE: line added 
    cursor = conn.cursor()
    try:
...
if __name__ == "__main__":
    ...
    evaluate(gold, pred, db_dir, etype, kmaps,fout)
