...
@Vocabulary.register("retriever_vocab",constructor="from_instances")
class RetriverSpiderVocabulary(Vocabulary):
    ...
    @classmethod
    def from_instances(cls,pkl_name: str,question_token_indexers: Dict[str, TokenIndexer]) -> "Vocabulary":
        print('new one')
        with open(pkl_name,'rb') as f:
            allinst=pickle.load(f)
        for instance in allinst:
            instance.fields["enc"].token_indexers = question_token_indexers 
        
        logger.info("Fitting token dictionary from dataset.")
        padding_token =  DEFAULT_PADDING_TOKEN
        oov_token =  DEFAULT_OOV_TOKEN
        namespace_token_counts: Dict[str, Dict[str, int]] = defaultdict(lambda: defaultdict(int))
        for instance in Tqdm.tqdm(allinst, desc="building vocab"):
            instance.count_vocab_items(namespace_token_counts)
