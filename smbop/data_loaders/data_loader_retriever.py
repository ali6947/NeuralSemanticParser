...
from allennlp.data.fields import  ListField, MetadataField,TensorField
...
@DataLoader.register("multiprocess_retriever")
class RetriverMultiProcessDataLoader(DataLoader):
    """
    ...
    def __init__(
        ...
        *,
        ...
        quiet: bool = False,
        nn_mask_file: str= None,
        all_instances: str=None,
        all_TED: str=None,
    ) -> None:
        ...
        print('loading nn mask')
        with open(nn_mask_file,'rb') as f:
            self.nn_masks=pickle.load(f)

        print('loading all instances')
        with open(all_instances,'rb') as f:
            self.all_instances=pickle.load(f)
        [self.reader.apply_token_indexers(x) for x in self.all_instances]

        print('loading all TEDs')
    ...
    def _instances_to_batches(
        ...
        ########## added by ali
        self.all_instances=[self._index_instance(instance) for instance in self.all_instances]
        [inst.add_field('inst_id',MetadataField(i)) for i,inst in enumerate(self.all_instances)]
        ##########
            ...
            for instances in instance_chunks:
                ...
                for batch in batches:
                    ...
                    for inst in batch:
                        ...
                        nn_inst=[self.all_instances[nn] for nn in chosen_inst]
                        ins_fields = {}
                        ...
                        ##### NOTE: latest addition 30/12/2021
                        TEDlist=[0]
