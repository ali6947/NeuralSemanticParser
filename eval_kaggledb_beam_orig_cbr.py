...
def main():
    ...
    # questions_path=questions_path.replace('_w_top5_cases_from_train_no_leaf_thresh_gt_0.7','') ## added for ali's setup
    # questions = [line.strip() for line in open(questions_path,'r')]
    ...
    with open(args.output, "w") as g:    
        with open(args.dev_path,'rb') as f:
            ...
            for i, el in enumerate(tqdm.tqdm(dev_pkl)):
                ...
                tokens_with_caseSQL=el['enc'].tokens.copy()

                if len(el['cases'].metadata)!=0:
                    pool = cycle(el['cases'].metadata)
                    for item in pool:
                ...
                el.add_field('cases',tokens_with_caseSQL) #NOTE: add_field overwrise the first argument ke name wali field, if already present
                el.fields["cases"].token_indexers = predictor._dataset_reader._utterance_token_indexers
                if i==0:
                    instance_0=deepcopy(el)
