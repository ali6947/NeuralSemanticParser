...
def main():
    ...
    BASE_PATH_DST="/mnt/infonas/data/alirehan/semantic_parsing/pickle_objs/bigbird_base/spider_val_cbr_splits_30"
    dirs1=['split_0', 'split_1', 'split_2']
    dirs2=['flight_2', 'car_1', 'cre_Doc_Template_Mgt', 'dog_kennels', 'world_1']

    for f1 in dirs1:
        for f2 in dirs2:
            test_file=os.path.join(BASE_PATH_DST,f1,f2,'test.pkl')
            cases_file=os.path.join(BASE_PATH_DST,f1,f2,'train.pkl')
            with open(test_file, "rb") as f:
                test_pkl=pickle.load(f)
            with open(cases_file, "rb") as f:
                cases_pkl=pickle.load(f)

            with open(f'results/bigbird_splits_v2/{f1}_{f2}_pred', "w") as g:
                with open(f'results/bigbird_splits_v2/{f1}_{f2}_gold','w') as gg:
                        set0=False
                        leaf_log=[]
                        avg_prec=[]
                        inv_rank=[]
